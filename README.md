...
在 Windows x64 上执行 `npm run package:release`，会生成带时间戳的 `release/WordAgent-2.2.0-windows-x64-*.zip`。包内自带 Node 运行时、生产依赖和构建文件，接收者无需安装开发环境。解压后双击 `Install.cmd`，后续使用 `Start.cmd` / `Stop.cmd`。
...
如旧加载项被缓存，关闭原窗格，重新加载根目录清单。清单版本已更新到 `2.2.0.0`，不再使用旧 `frontend/office-addin` 路径。
...
设置中可分别测试普通连接和 Agent 工具能力、调整温度与最大输出、切换默认模型和删除密钥。普通连接测试只发送 `Reply with OK.`；Agent 工具测试只发送一条要求调用无参数 `wordagent_capability_probe` 的测试消息并强制该函数，不发送文档。两种测试都不证明模型的编辑质量。
...
3. 实时应用默认开启，也可在输入框上方关闭，改为审阅后应用。只有完整且通过校验的一批编辑才能写入；本机工具生成的批量替换始终先预览，不受实时应用开关影响。
4. 变更记录保存每批操作与状态。当前打开期间最近一批修改可撤回；后续手工修改会阻止直接撤回，以免覆盖新内容。

支持段落替换、精确短语替换、插入、删除，原始选区替换，标题样式、加粗、斜体、字号、字体、对齐，以及简单表格插入。每一批编辑先核对完整文档版本和目标原文，同一段落不能在同一批次重复操作。冲突时重新生成，不会强行覆盖。
...
打开侧边栏“文档工具”：无需模型即可查找正文、跳转段落、预览精确批量替换，以及检查标题跳级、空标题、待补充内容和长段落重复。查找区分大小写、按字面子串匹配，不是正则或完整单词匹配；包含受保护内容时替换整批拒绝，不会跳过后仍声称全部完成。

八个 AI 写作入口覆盖校对润色、精简表达、中译英、要点摘要、逻辑审阅、提取待办、整理表格、术语一致性。点击只准备提示词，不自动发送或修改文档；仍需你确认范围并发送。

Agent 全文模式现在可调用 `inspect_document`、`search_document`，再通过 `replace_in_document` 或 `edit_document` 生成一次最终方案；界面显示工具执行记录。每次最多四轮只读工具查询、五次模型请求，整个回合共用 180 秒期限。Ask 仍不发送工具；选区模式不暴露全文工具。模型调用费用可能随查询轮数增加。

短语替换使用 `replace_text`，保留匹配范围之外的富文本；指定 `occurrence` 可只替换段落中的第 N 次出现。最多 60 段、1,000 处匹配，查找文本最多 120 字符，局部替换文本最多 2,000 字符，不允许跨换行。详情、限制与官方技术依据见 [文档工具说明](docs/document-tools.md)。
